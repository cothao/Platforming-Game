...
platforms = [
    new Platform({
        position: {
            x: platformImage.width * 4 + 300 -1 + platformImage.width - platformSmallTallImage.width, // multiply by each platform generated
            y: 270
    },
...
const animate = function() {
    ...
    if (keys.right.pressed && player.position.x < 400) { // if my position is between a certain gap, than I am free to move, otherwise parallax screen initiates
        ...
        if (keys.right.pressed) {
            ...
            genericObjects.forEach(genericObject => {
                genericObject.position.x -= player.speed * .66 // make slower for nice parallax speed
            })
